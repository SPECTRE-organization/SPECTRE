...
function setcopy(text, alertmsg){
	if(is_ie) {
		...
		$('floatwin_clipboard_title').innerHTML = 'คลิปบอร์ด';
		str = '<div style="text-decoration:underline;">คลิ๊กเพื่อคัดลอกไปที่คลิ๊ปบอร์ด</div>' +
...
function dconfirm(msg, script, width, height) {
	...
	$('floatwin_confirm_title').innerHTML = 'ข้อความ';
	$('floatwin_confirm_content').innerHTML = msg + '<br /><button onclick="' + script + ';floatwin(\'close_confirm\')">&nbsp;ใช่&nbsp;</button>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<button onclick="floatwin(\'close_confirm\')">&nbsp;ไม่&nbsp;</button>';
}
...
function dnotice(msg, script, width, height) {
	...
	$('floatwin_confirm_title').innerHTML = 'ข้อความ';
	$('floatwin_confirm_content').innerHTML = msg + (script ? '<br /><button onclick="' + script + ';floatwin(\'close_confirm\')">ปิด</button>' : '');
}
...
function showloading(display, waiting) {
	...
	var waiting = waiting ? waiting : 'กำลังโหลด...';
...
function floatwin(action, script, w, h, scrollpos) {
	...
	if(action == 'open') {
		...
		if(!$(layerid)) {
			...
			$(layerid).innerHTML = '<div><h3 class="float_ctrl"><em><img src="' + IMGDIR + '/loading.gif"> กำลังโหลด...</em><span><a href="javascript:;" class="float_close" onclick="floatwinreset = 1;floatwin(\'close_' + handlekey + '\');">&nbsp</a></span></h3></div>';
...
function showselect(obj, inpid, t, rettype) {
	if(!obj.id) {
		...
		if(!t) {
			...
			s += showselect_row(inpid, 'เลือกเอง', -2);
		} else {
			if($(t)) {
				...
				s += showselect_row(inpid, 'เลือกเอง', -1);
			} else {
				s += '<a href="javascript:;" onclick="$(\'' + inpid + '\').value = \'0\'">ถาวร</a><br />';
				...
				s += showselect_row(inpid, '6 เดือน', 182, 1, rettype);
...
function showimmestatus(imme) {
	var lang = {'Online':'ออนไลน์','ไม่ว่าง':'เดี๋ยวกลับมา','ไม่อยู่':'ติดสาย','พักกลางวัน':'ออฟไลน์'};
