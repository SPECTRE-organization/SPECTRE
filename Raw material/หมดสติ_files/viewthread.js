/*
	...
	$Id: viewthread.js 18204 2009-03-30 05:39:35Z monkey $
*/
...
function copycode(obj) {
	setcopy(is_ie ? obj.innerText.replace(/\r\n\r\n/g, '\r\n') : obj.textContent, 'คัดลอกไปที่คลิปบอร์ด');
}
...
function zoomST(c) {
	if($('zoomimglayer').style.display == '') {
		...
		if(c <= 5) {
			...
			$('zoomimglayer').innerHTML = '<div class="zoominner"><p><span class="right"><a href="' + zoomobj['zimg'] + '" class="imglink" target="_blank" title="เปิดหน้าต่างใหม่">เปิดหน้าต่างใหม่</a><a href="javascipt:;" onclick="zoomimgadjust(event, 1)" class="imgadjust" title="ขนาดจริง">ขนาดจริง</a><a href="javascript:;" onclick="zoomclose()" class="imgclose" title="ปิด">ปิด</a></span>หมุนล้อที่เมาส์เพื่อซูมภาพ</p><div id="zoomimgbox"><img id="zoomimg" style="cursor: move;" src="' + zoomobj['zimg'] + '" width="' + parseInt($('zoomimglayer').style.width) + '" height="' + parseInt($('zoomimglayer').style.height) + '"></div></div>';
...
function setanswer(pid){
	if(confirm('You sure you want to put the replies to the selected as "the best answer"?')){
		if(is_ie) {
			doane(event);
		}
...
function fastpostvalidate(theform) {
	...
	if(theform.message.value == '' && theform.subject.value == '') {
		s = 'กรุณาใส่หัวข้อและเนื้อหา';
		theform.message.focus();
	} else if(mb_strlen(theform.subject.value) > 250) {
		s = 'หัวข้อเกิน 80 ตัวอัพษร';
		theform.subject.focus();
	}
	if(!disablepostctrl && ((postminchars != 0 && mb_strlen(theform.message.value) < postminchars) || (postmaxchars != 0 && mb_strlen(theform.message.value) > postmaxchars))) {
		s = 'ความยาวของบทความของคุณเกินที่ระบบกำหนดไว้.\n\nความยาวปัจจุบัน: ' + mb_strlen(theform.message.value) + ' ' + 'ไบต์\nระบบจำกัดที่: ' + postminchars + ' ถึง ' + postmaxchars + ' ไบต์';
	}
	if(s) {
...
function submithandle_fastpost(locationhref) {
	...
	if(pid != -1) {
		...
		$('fastpostreturn').innerHTML = 'โพสต์บอร์ดนี้ต้องตอบรับการอนุมัติการโพสต์ของคุณจะปรากฏหลังจากผ่านการตรวจสอบ';
	}
}
